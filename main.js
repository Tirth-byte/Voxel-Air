import { BrandOverlay } from './js/ui/BrandOverlay.js';
import { COLORS, createPremiumVoxelMaterial, createPremiumWireframeMaterial } from './js/ui/UITheme.js';
import { createWorkspaceFrame, requestFullscreen } from './js/ui/InteractionUtils.js';

// Global Three.js access
...
// Branding
const branding = new BrandOverlay(camera);
...
window.addEventListener('resize', () => {
    ...
    // Update canvas sizes for MediaPipe
...
if (btnPhone) {
    btnPhone.addEventListener('click', () => {
        ...
        btnPhone.classList.toggle('active', isPhoneMode);

        if (isPhoneMode) {
            // Apply Phone Mode Settings
            camera.fov = 60; // Wider FOV for closer feeling
            camera.position.z = 25; // Move back slightly to see more
            gridSize = 1.5; // Larger voxels for touch feeling
        } else {
            // Revert
            camera.fov = 45;
            camera.position.z = 20;
        ...
        crosshair.geometry = new THREE.BoxGeometry(gridSize, gridSize, gridSize);
    });
}
...
function onResults(results) {
    ...
    if (rHand) {
        ...
        if (isBuilding || buildTimer > 0 || isErasing || eraseTimer > 0) {
            ...
            crosshair.material.color.set((isErasing || eraseTimer > 0) ? 0xff3333 : COLORS.VOXEL_GREEN);

            // Re-scale crosshair if phone mode changed gridSize
            if (Math.abs(crosshair.geometry.parameters.width - gridSize) > 0.01) {
                crosshair.geometry.dispose();
                crosshair.geometry = new THREE.BoxGeometry(gridSize, gridSize, gridSize);
            }
        }
...
function addSketchVoxel(x, y, z) {
    ...
    const mesh = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: COLORS.VOXEL_GREEN, wireframe: true, transparent: true, opacity: 0.3 })
    );
...
function createFinalCube(x, y, z) {
    ...
    // Use Premium Material from Theme
    const m = createPremiumVoxelMaterial();
    const mesh = new THREE.Mesh(g, m);
    mesh.position.set(x, y, z);
    mesh.origin = new THREE.Vector3(x, y, z);
    mesh.velocity = new THREE.Vector3(0, 0, 0);

    const edges = new THREE.LineSegments(new THREE.EdgesGeometry(g), createPremiumWireframeMaterial());
    mesh.add(edges);
    return mesh;
}
...
const cameraObj = new Camera(videoElement, { onFrame: async () => { bioCanvas.width = videoElement.videoWidth; bioCanvas.height = videoElement.videoHeight; await hands.send({ image: videoElement }); }, width: 1280, height: 720 });
cameraObj.start();
